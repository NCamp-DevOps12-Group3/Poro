...
public class DdayCalculator {
    ...
    public String getDdayCalculator(LocalDateTime regdate) {
        ...
        // ★ 남은 일수 계산(dday 계산기)
        long totalDays = ChronoUnit.DAYS.between(presentTime.toLocalDate(), regdate.toLocalDate());
        long minute = ChronoUnit.MINUTES.between(presentTime, regdate) % 60;
        long hour = ChronoUnit.HOURS.between(presentTime, regdate) % 24;
